function [tf_pow, varargout] = tfdecomp_Z_lin(cfg,eegdat,varargin)
...
% check output dir
if ~exist(writdir,'dir') %%检查用于保存数据的文件夹是否存在，不存在就创建一个
    mkdir(writdir)
end
...
wavelets = zeros(nfreqs,length(t)); %%wavelets
for fi=1:nfreqs
    wavelets(fi,:)=exp(2*1i*pi*frex(fi).*t).*exp(-t.^2./(2*s(fi)^2));
end

fre_index = dsearchn(frex',smale_fre');
wavelets = wavelets(fre_index(1):fre_index(end),:);
frex = frex(fre_index(1):fre_index(end));
nfreqs = length(frex);
...
for condi=1:nconds
    ...
    %fre = [60 140];% gamma 40 - 140
    %Z_data = squeeze(mean(zscore(abs(rawconv).^2,1,3),2));
  %tf_pow{condi} = squeeze(mean(abs(rawconv).^2,4));
   %fre_power{condi} = squeeze(mean(zscore(abs(rawconv).^2,1,3),2));
   %fre_p{condi}=squeeze(zscore(abs(rawconv).^2,1,3));%power
    %fre_phase{condi}  = angle(rawconv);
    tf_raw{condi}  =rawconv;
...
%     if connectivity
%         if sum(basetime)
%             save(outputfilename,'tf_pow','tf_phase','tf_sync','dim','tf_pow_single');
%         else
%             save(outputfilename,'tf_sync','dim','tf_pow_single');
%         end
%     elseif singletrial
%         save(outputfilename,'tf_pow','dim','tf_pow_single');
%     elseif robfit
%         if sum(basetime)
%             save(outputfilename,'tf_rvals','tf_pow','tf_phase','dim','tf_pow_single');
%         else
%             save(outputfilename,'tf_rvals','dim','tf_pow_single');
%         end
%     else
%         save(outputfilename,'tf_pow','tf_phase','dim','tf_pow_single');
%     end
% end

save(outputfilename,'tf_raw','dim','-v7.3');

% if save_output
%     if connectivity
%         if sum(basetime)
%             save(outputfilename,'fre_power','tf_phase','tf_sync','dim','baselinedata');
%         else
%             save(outputfilename,'tf_sync','dim','baselinedata');
%         end
%     elseif singletrial
%         save(outputfilename,'fre_power','dim','baselinedata');
%     elseif robfit
%         if sum(basetime)
%             save(outputfilename,'tf_rvals','tf_pow','tf_phase','dim','baselinedata');
%         else
%             save(outputfilename,'tf_rvals','dim','baselinedata');
%         end
%     else
%        %  save(outputfilename,'fre_power','dim','-v7.3');
%         save(outputfilename,'tf_raw','dim','-v7.3');
%     end
% end

%     for i = 1:8  %将数据fre_phase内含的各个位置分开
%         eval([ 'location_' num2str(i) '_data_fre =fre_phase{' num2str(i) '};']);
%     end
%    tf_raw=[];
%     for i = 1:8 %各位置数据合并，放在tf_raw
%         eval(['tf_raw = cat(4,tf_raw,location_' num2str(i ) '_data_fre);'])
%     end
% 
%   save(outputfilename,'tf_raw','dim','-v7.3');
%%
